...
### Added
- **Electrical readings for WallSwitch and Socket-family devices.** Each WallSwitch / Socket / `relay` / `relay_fibra_base` / `socket_b` / `socket_g` / `socket_outlet_type_e` / `socket_outlet_type_f` / `socket_type_g_plus` now exposes three sensors that mirror what the official Ajax PRO app shows on the device card: `sensor.<name>_current` (A, `device_class=current`, `state_class=measurement`, scaled from milliamps), `sensor.<name>_energy_consumed` (kWh, `device_class=energy`, `state_class=total_increasing` — wires into HA's Energy dashboard with proper meter-reset semantics, scaled from Wh), and `sensor.<name>_power_derived` (W, `device_class=power`, opt-in / disabled by default, derived as `current × NOMINAL_GRID_VOLTAGE_V` since the WallSwitch firmware does not report measured voltage — same approach as the official app). Values arrive through HTS in the `current_ma` (sub-key `0x42`) and `power_consumed_wh` (sub-key `0x43`) per-device TLV fields; the consumer path was identified out of Ajax PRO 2.47 (`poz0` / `Le4` / `Realm AXDevice` → `ckh.mapCommonElectricPowerProps`) and confirmed against @brunovdw68's HTS debug log on #123. (#123, #137, surfaced by @brunovdw68 in #123)
- **Per-device kv extraction in HTS bodies.** New `_extract_all_devices_kv` walks the entire `STATUS_BODY` / `SETTINGS_BODY` payload and emits one `(device_id, kv)` tuple per device marker — previously the parser only extracted the hub's row and dropped every other device's data silently. Used by the readings parser above and by a DEBUG-only `#123 probe` log line that catalogues which sub-keys non-hub devices carry, making the audit of an unfamiliar device family one log sample away. (#137)
...
### Changed
- **HTS `STATUS_UPDATE` (sub-key `0x0b`) and `SETTINGS_UPDATE` (`0x0c`) are now consumed in place.** Both are per-device delta pushes from the hub; payload shape is `[sub_key, device_id_4b, k1, v1, k2, v2, …]` — identical to one device's row inside `STATUS_BODY`. Routed through the same `on_device_kv` callback the readings parser uses for the boot snapshot, so live electrical readings feel near-instant (whatever debounce window the hub applies) instead of waiting for the next periodic refresh. The audit on Ajax PRO 2.47 showed that the explicit `SUBSCRIBE_UPDATES` / `BULK_SUBSCRIBE_UPDATES` msg-types live inside the deprecated `ug8` class and have no live callers — the auth handshake is the implicit subscription. Subsumes the silent-drop behaviour added in `1.3.0-beta.7` (#128 / #111): the problem there was firing `_schedule_hub_refresh` on every heartbeat (~8.6 KB round-trip), not the drop itself; we now read the delta in-place and never schedule a refresh from it. (#137)

### Internal
- Full `ctw0` HTS msg-type vocabulary now mapped (`SETTINGS_BODY`, `STATUS_BODY`, `STATUS_UPDATE`, `SETTINGS_UPDATE`, `REQUEST_FULL_STATUS`, `SUBSCRIBE_UPDATES`, `BULK_SUBSCRIBE_UPDATES`, `FORCE_CHECK`, `GET_FULL_DATA`, `END_FULL_DATA`, `GET_HUB_PARAMS`, `HUB_PARAMS`, `BREAK_UPDATE`, `CUSTOM_SELECTION`, `BULK_READ_HUB_PARAMS`, `HUB_SETTINGS_SEARCH`) — captured in maintainer memory so the next protocol question has a ready reference instead of another round of `apktool d`.
...
- README "How to obtain FCM credentials" no longer sends users hunting for `google_api_key` in `strings.xml` — three of the four values live there but the api_key is in `lib/<arch>/libnative-lib.so`. (#83)
