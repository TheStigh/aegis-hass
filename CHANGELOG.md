...
### Added
- **Outlet Type E / Type F electrical sensors enabled with their own sub-key mapping** (#179, calibrated from @SaetanSaDiablo's four-load reboot capture). The Outlet family was excluded from `ELECTRICAL_DEVICE_TYPES` since `1.5.3-beta.1` because its STATUS_BODY sub-key layout differs from the WallSwitch family and no load-calibrated capture had landed. With known loads applied at HA restart (off / idle / 15 W / ~2 kW), the readings line up unambiguously: power and current both lined up against 230 V mains (15 W / 230 V ≈ 65 mA matches the raw value × 10 mA scale; 2 080 W / 230 V ≈ 9.04 A matches the raw 893 × 10 mA = 8.93 A), the energy counter increments under load, and voltage tracks the expected EU mains range. `parse_device_readings` now dispatches on device_type through a per-family sub-key table (`_WALLSWITCH_KEY_MAP` / `_OUTLET_KEY_MAP`) so each family stays isolated and adding a third family is a one-entry change. WallSwitch behaviour is unchanged.
- **New `power` sensor for Outlet Type E / F.** Outlet firmware reports instantaneous power directly, so the integration surfaces it as a real entity (enabled by default, `SensorDeviceClass.POWER`) rather than the derived-from-current placeholder the WallSwitch family uses. Voltage on this family is now a real reading too — no `NOMINAL_GRID_VOLTAGE_V` fallback needed for these devices. Entity unique_id is `aegis_ajax_<device_id>_power`, distinct from the WallSwitch family's `_power_derived`. Translations land in all 14 locales.
